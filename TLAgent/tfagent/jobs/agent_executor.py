...
from tfagent.llms.local_llm import LocalLLM

import tfagent.worker
from tfagent.agent.agent_iteration_step_handler import AgentIterationStepHandler
from tfagent.agent.agent_tool_step_handler import AgentToolStepHandler
from tfagent.agent.agent_workflow_step_wait_handler import AgentWaitStepHandler
from tfagent.agent.types.wait_step_status import AgentWorkflowStepWaitStatus
from tfagent.apm.event_handler import EventHandler
from tfagent.config.config import get_config
from tfagent.lib.logger import logger
from tfagent.llms.google_palm import GooglePalm
from tfagent.llms.hugging_face import HuggingFace
from tfagent.llms.llm_model_factory import get_model
from tfagent.llms.replicate import Replicate
...
from tfagent.models.workflows.agent_workflow_step import AgentWorkflowStep
from tfagent.models.workflows.agent_workflow_step_wait import AgentWorkflowStepWait
...
from tfagent.worker import execute_agent_serial
from tfagent.agent.types.agent_workflow_step_action_types import AgentWorkflowStepAction
from tfagent.agent.types.agent_execution_status import AgentExecutionStatus

# from tfagent.helper.tool_helper import get_tool_config_by_key

engine = connect_db()
# Session是一个session创建函数，每调用一次会产生一个新的session对象。
...
class AgentExecutor:
    ...
    def execute_next_step(self, agent_execution_id):
        ...
        try:
            ...
            try:
                ...
                #                                                                           model_api_key))
                # else:
                # vector_store_type = VectorStoreType.get_vector_store_type(VectorStoreType.CHROMA)
                # memory = VectorFactory.get_vector_storage(vector_store_type, "super-agent-index1",
                #                                           AgentExecutor.get_embedding("HuggingLocal",
                #                                                                       model_api_key,
            ...
            try:
                ...
                tfagent.worker.execute_agent.apply_async((agent_execution_id, datetime.now()), countdown=15)
                # tfagent.worker.execute_agent_serial(agent_execution_id, datetime.now())
            ...
            if agent_execution.status == "COMPLETED" or agent_execution.status == "WAITING_FOR_PERMISSION":
                ...
                return
            # tfagent.worker.execute_agent_serial(agent_execution_id, datetime.now())
            tfagent.worker.execute_agent.apply_async((agent_execution_id, datetime.now()), countdown=2)
        finally:
            session.close()
            engine.dispose()
    ...
    @classmethod
    def get_embedding(cls, model_source, model_api_key, model_name=None):
        ...
        if "HuggingLocal" in model_source:
            if model_name is not None:
                return HuggingFaceLocalEmbed(model_name=model_name)
        elif "Hugging" in model_source:
        ...
        for agent_execution in waiting_agent_executions:
            ...
            if step_wait is not None:
                ...
                if ((datetime.now() - step_wait.wait_begin_time).total_seconds() > wait_time
                    ...
                    execute_agent_serial(agent_execution.id, datetime.now())
        session.close()