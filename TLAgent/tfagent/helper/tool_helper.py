...
from tfagent.models.tool_config import ToolConfig
from tfagent.models.toolkit import Toolkit
from tfagent.tools.base_tool import BaseTool, ToolConfiguration
from tfagent.tools.base_tool import BaseToolkit
    ...
    for folder_path in folder_paths:
        ...
        for folder_name in os.listdir(folder_path):
            ...
            if not os.path.isdir(folder_dir):
                continue
                # sys.path.append(os.path.abspath('tfagent/tools/email'))
...
def register_toolkits(session, organisation, tool_paths=None):
    if tool_paths is None:
        tool_paths = ["tfagent/tools", "tfagent/tools/external_tools"]

    # if get_config("ENV", "DEV") == "PROD":
    #     tool_paths.append("tfagent/tools/marketplace_tools")
...
def register_marketplace_toolkits(session, organisation):
    tool_paths = ["tfagent/tools", "tfagent/tools/external_tools","tfagent/tools/marketplace_tools"]
    if organisation is not None:
        process_files(tool_paths, session, organisation)
...
def handle_tools_import():
    ...
    tool_paths = ["tfagent/tools", "tfagent/tools/marketplace_tools", "tfagent/tools/external_tools"]
