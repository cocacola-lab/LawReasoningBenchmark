...
class FactFindingHeadSchema(BaseModel):
    pass
...
class FactFindingHeadTool(BaseTool, ABC):
    """
    Fact finding head tool
    ...
    name = "FactFindingHeadTool"
    description = (
        "Intelligent fact finding assistant that comprehends the contents of the document, find out criminal fact from given data only."
    )
    args_schema: Type[FactFindingHeadSchema] = FactFindingHeadSchema
    goals: List[str] = []
    ...
    def _execute(self, ):
        ...
        """
        try:
            prompt = PromptReader.read_tools_prompt(__file__, "fact_finding_head.txt")
