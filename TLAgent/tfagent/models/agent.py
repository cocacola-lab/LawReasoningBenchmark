...
from tfagent.models.agent_template_config import AgentTemplateConfig
# from tfagent.models import AgentConfiguration
...
class Agent(DBBaseModel):
    ...
    @classmethod
    def get_last_agent_by_project_and_name(cls, session, project_id: int, agent_name: str):
        db_agent = (session.query(Agent).filter(Agent.name == agent_name, Agent.project_id == project_id,
                                               or_(Agent.is_deleted == False, Agent.is_deleted == None))
                                               .order_by(Agent.id.desc()).first())  # 取第一条（即最大的 ID，即最后创建的记录）
        return db_agent
