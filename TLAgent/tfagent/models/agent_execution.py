...
from tfagent.models.base_model import DBBaseModel
...
class AgentExecution(DBBaseModel):
    ...
    @classmethod
    def get_agent_execution_by_name(cls, session, agent_execution_name):
        """
            Get Agent by agent execution name

            Args:
                session: The database session.
                agent_execution_name(str) : Unique identifier of an Agent Execution.

            Returns:
                AgentExecution: AgentExecution object is returned.
        """
        return session.query(AgentExecution).filter(AgentExecution.name == agent_execution_name).first()

    @classmethod
    def get_agent_execution_by_agent_id_and_name(cls, session, agent_id, agent_execution_name):
        """
            Get Agent by agent id and agent execution name
            ...
            Returns:
                AgentExecution: AgentExecution object is returned.
        """
        return session.query(AgentExecution).filter(AgentExecution.agent_id == agent_id, AgentExecution.name == agent_execution_name).first()