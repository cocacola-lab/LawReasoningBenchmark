...
from sqlalchemy import Column, Integer, Text, String, exists
from typing import Union

from tfagent.config.config import get_config
from tfagent.helper.encyption_helper import decrypt_data
...
from tfagent.types.model_source_types import ModelSourceType
...
#         return get_config("OPENAI_API_KEY")
    ...
    @classmethod
    def agent_config_exists(cls, session, agent_id: int):
        is_exists = session.query(
            session.query(AgentConfiguration).filter(AgentConfiguration.agent_id == agent_id).exists()
        ).scalar()
