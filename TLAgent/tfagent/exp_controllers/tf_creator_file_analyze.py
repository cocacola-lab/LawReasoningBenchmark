...
from tfagent.models.resource import Resource

from tfagent.types.storage_types import StorageType
from tfagent.config.config import get_config
from tfagent.helper.resource_helper import ResourceHelper

# 设置工作路径
os.chdir("./PyProject/Law_TF_refine")
...
parser.add_argument("--workflow", type=str, default="split json items",
                    choices=["Goal Based Workflow", "Dynamic Task Workflow", "Fixed Task Workflow", "Read File Workflow", "Read File Workflow2", "split json items"],
                    help='Agent description for the script.')

# file dir
parser.add_argument('--file_path', type=str, default="",
                    help='input file.')
parser.add_argument('--dir_path', type=str, default="D:/test2",
...
def uploadfile(upload_file_path, agent):
    pattern = r'[\\/]'
    filename = re.split(pattern, upload_file_path)[-1]
    # MB
    size = os.stat(upload_file_path).st_size
    # file type
    type = "application/" + filename.split('.')[-1]

    with open(upload_file_path, "rb") as f:
        file = f.read()

    accepted_file_types = (".pdf", ".docx", ".pptx", ".csv", ".txt", ".epub")
    if not upload_file_path.endswith(accepted_file_types):
    ...
    if "{agent_id}" in save_directory:
        save_directory = ResourceHelper.get_formatted_agent_level_path(agent=Agent
                                                                       .get_agent_from_id(session=session,
                                                                                          agent_id=agent.id),
                                                                       path=save_directory)
    ...
    resource = Resource(name=filename, path=file_path, storage_type=storage_type.value, size=size, type=type,
                        channel="INPUT",
                        agent_id=agent.id)
...
def run_tfagent_cli(agent_name=None, agent_description=None, agent_goals=None):
    ...
    if args.file_path != "":
        uploadfile(args.file_path, agent)
    elif args.dir_path != "":
        file_list = os.listdir(args.dir_path)
        for file in file_list:
            uploadfile(os.path.join(args.dir_path, file), agent)
