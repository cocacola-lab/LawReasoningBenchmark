...
from tfagent.helper.auth import get_current_user, check_auth
from tfagent.models.tool_config import ToolConfig
from tfagent.models.toolkit import Toolkit
from tfagent.models.oauth_tokens import OauthTokens
...
async def google_auth_calendar(code: str = Query(...), state: str = Query(...)):
    ...
    if env == "DEV":
        ...
        redirect_uri = "https://app.tfagent.com/api/google/oauth-tokens"
    params = {
        'client_id': client_id,
    ...
    frontend_url = tfagent.config.config.get_config("FRONTEND_URL", "http://localhost:3000")
