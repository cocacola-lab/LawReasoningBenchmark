...
parser.add_argument("--workflow", type=str, default="Fact Workflow 2",
                    choices=["Goal Based Workflow", "Dynamic Task Workflow", "Fixed Task Workflow", "Fact Workflow 2"],
                    help='Agent description for the script.')

# The path to the experiment file
parser.add_argument("--input-dir", type=str, default="./dataproc/splitdata/task1",)
...
def run_tfagent_cli(agent_name=None, agent_description=None, agent_goals=None):
    ...
    agent_execution_configs = {
        ...
        "user_timezone": None,
        "knowledge": agent_config_values["knowledge"],
    }
...
def input_data_formulation(item):
    if "fact" in args.workflow.lower():
        template = readjson("input_template/fact_template.txt")
        template["agent_goals"] = template["agent_goals"].replace("{legal_text}", item["case_info"])
    else:
        template = ""
...
def main():
    ...
    for data_file in dataset_list:
        data_items = readjson(os.path.join(args.input_dir, data_file))
        for item in data_items["case_segments"]:
            input_forum = input_data_formulation(item)
            agent_name = "{}_{}_{}".format(input_forum["agent_name"], data_items["id"], item["seg"])
            run_tfagent_cli(agent_name=agent_name,
                            agent_description=input_forum["agent_description"],
